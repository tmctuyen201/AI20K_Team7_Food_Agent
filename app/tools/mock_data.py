"""Mock data for 10 sample users (Phase 1 — no real GPS)."""
...
def get_mock_location(user_id: str) -> dict:
    """Return lat/lng/city for a known user_id, or a default."""
    for user in MOCK_USERS:
        if user["user_id"] == user_id:
            return user

    # Fallback to u01 (Hà Nội) for unknown IDs
    logger.warning("mock_location_fallback", user_id=user_id, fallback="u01")
    return MOCK_USERS[0]